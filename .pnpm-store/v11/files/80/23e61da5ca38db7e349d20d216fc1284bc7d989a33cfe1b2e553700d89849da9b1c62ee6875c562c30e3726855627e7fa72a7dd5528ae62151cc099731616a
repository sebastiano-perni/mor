...
function combineValues({ resolvedData, resolvedValue, separator, context, parentSchema }) {
	...
	if (separator === "allOf") {
		...
		const pickableRequiredProperties = overrideRequiredProperties.filter((prop) => resolvedData.allProperties.includes(prop));
		const unresolvedRequiredProperties = overrideRequiredProperties.filter((prop) => !resolvedData.allProperties.includes(prop));
		let result = joined;
